...
class RTGEncStack(BertPreTrainedModel):

    def __init__(self, config, add_pooling_layer=False):
        super().__init__(config)
        self.config = config

        self.embeddings = RTGEmbeddings(config)
        self.encoder = RTGEncoder(config)
...
class RTGEncForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        ...
        self.sent_rpr_attn = nn.MultiheadAttention(config.hidden_size,
                                                   config.num_attention_heads,
                                                   dropout=config.attention_probs_dropout_prob,
                                                   batch_first=True)
        self.sent_rpr_query = nn.Parameter(torch.zeros(config.hidden_size))
...
class RTGEmbeddings(nn.Module):
    ...
    def __init__(self, config, layernorm_embed=False):
        ...
        # self.token_type_embeddings = torch.zeros(config.type_vocab_size, config.hidden_size, device=self.position_ids.device)
...
class RTGEncLayer(nn.Module):
    def __init__(self, config):
        ...
        self.ff_dense = nn.Linear(config.intermediate_size, config.hidden_size)
    ...
    def forward(
        ...
        output = self.attn_layernorm(hidden_states)
        ...
        attention_output = hidden_states + self.attn_dropout(self.attn_dense(attention_output))
        # attention_output = (attention_output,) + self_attention_outputs[1:]
    ...
    def feed_forward_chunk(self, attention_output):
        output = self.ff_layernorm(attention_output)
        intermediate_output = self.intermediate(output)
        layer_output = attention_output + self.ff_dropout(self.ff_dense(intermediate_output))
        return layer_output
...
class RTGEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RTGEncLayer(config) for _ in range(config.num_hidden_layers)])
